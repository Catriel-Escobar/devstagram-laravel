...
<body class="bg-gray-100">
    ...
    <header class="bg-white p-5 border-b shadow flex justify-between">
        ...
        <nav class="flex gap-2 items-center">
            ...
            <a href="{{ route('posts.create') }}" class="flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold cursor-pointer">
                ...
                <a href="{{ route('posts.index', ['user' => auth()->user()->username]) }}" class="font-bold  text-gray-600">
                Hola
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <input type="submit" class="font-bold uppercase text-gray-600 cursor-pointer" value="Cerrar sesion">
            </form>
        </nav>
        @endauth
    ...
    <footer class="text-center p-5 text-gray-500 font-bold uppercase mt-10">
        DevStagram - {{now()->year}} Todos los derechos reservados
